...
sub from
{
	...
	my $order = $self->{session}->param( '_buffer_order' );

	unless( defined $order )
	{
		my $url = $self->redirect_to_me_url();
		$url .= '&_buffer_order=-lastmod';
...
sub get_filters
{
        ...
        my $pref = $self->{id}."/shelf_access";
        my $user = $self->{session}->current_user;
        my @f = @{$user->preference( $pref ) || []};
        if( !scalar @f )
        {
		@f = ( adminids=>1, editorids=>1, readerids=>1 );
        }
	...
	foreach my $f ( qw/ adminids editorids readerids / )
        {
                ...
                $a->appendChild( $session->html_phrase( "eprint_fieldopt_shelf_access_$f" ) );
